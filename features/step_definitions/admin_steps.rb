...
# Returns a OAuth2 token associated with email "randomemail@gmail.com"
Given /I have a non-admin, unregistered Clever email/ do
...
Then /I can log in with Clever/ do
  allow(Teacher).to receive(:validate_auth).and_return(true)
  Rails.application.env_config["omniauth.auth"] = OmniAuth.config.mock_auth[:clever]
  page.find('button', text: /.*Sign in with Clever/).click()
end
