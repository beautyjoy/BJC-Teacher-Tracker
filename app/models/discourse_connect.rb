...
require_relative '../../lib/discourse_connect_base'
...
class DiscourseConnect < DiscourseConnectBase
  ...
  def self.generate_sso(return_path = "/", secure_session:)
    ...
    sso.return_sso_url = 'http://localhost:3000' + "/session/sso_login"
    sso
  end
  ...
  def nonce_error
    if Rails.cache.read(used_nonce_key).present?
